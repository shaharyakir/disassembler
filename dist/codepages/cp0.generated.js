...
CP0Auto.insertHex('8E', 7, (slice, indent) => {
    ...
    return `<{\n${(0, disassembler_1.decompile)(subslice, indent + 2)}${new Array(indent).fill(' ').join('')}}> PUSHCONT`;
});
...
CP0Auto.insertHex('db3c', 16, (slice, indent) => {
    ...
    return `<{\n${(0, disassembler_1.decompile)(subslice, indent + 2)}${new Array(indent).fill(' ').join('')}}> CALLREF`;
});
CP0Auto.insertHex('db3d', 16, (slice, indent) => {
    let subslice = slice.readRef();
    return `<{\n${(0, disassembler_1.decompile)(subslice, indent + 2)}${new Array(indent).fill(' ').join('')}}> JMPREF`;
});
CP0Auto.insertHex('db3e', 16, (slice, indent) => {
    let subslice = slice.readRef();
    return `<{\n${(0, disassembler_1.decompile)(subslice, indent + 2)}${new Array(indent).fill(' ').join('')}}> JMPREFDATA`;
});
...
CP0Auto.insertHex('e300', 16, (slice, indent) => {
    let subslice = slice.readRef();
    return `<{\n${(0, disassembler_1.decompile)(subslice, indent + 2)}${new Array(indent).fill(' ').join('')}}> IFREF`;
});
CP0Auto.insertHex('e301', 16, (slice, indent) => {
    let subslice = slice.readRef();
    return `<{\n${(0, disassembler_1.decompile)(subslice, indent + 2)}${new Array(indent).fill(' ').join('')}}> IFNOTREF`;
});
CP0Auto.insertHex('e302', 16, (slice, indent) => {
    let subslice = slice.readRef();
    return `<{\n${(0, disassembler_1.decompile)(subslice, indent + 2)}${new Array(indent).fill(' ').join('')}}> IFJMPREF`;
});
CP0Auto.insertHex('e303', 16, (slice, indent) => {
    let subslice = slice.readRef();
    return `<{\n${(0, disassembler_1.decompile)(subslice, indent + 2)}${new Array(indent).fill(' ').join('')}}> IFNOTJMPREF`;
});
...
CP0Auto.insertHex('e30d', 16, (slice, indent) => {
    let subslice = slice.readRef();
    return `<{\n${(0, disassembler_1.decompile)(subslice, indent + 2)}${new Array(indent).fill(' ').join('')}}> IFREFELSE`;
});
CP0Auto.insertHex('e30e', 16, (slice, indent) => {
    let subslice = slice.readRef();
    return `<{\n${(0, disassembler_1.decompile)(subslice, indent + 2)}${new Array(indent).fill(' ').join('')}}> IFELSEREF`;
});
CP0Auto.insertHex('e30f', 16, (slice, indent) => {
    let subslice = slice.readRef();
    return `<{\n${(0, disassembler_1.decompile)(subslice, indent + 2)}${new Array(indent).fill(' ').join('')}}> IFREFELSEREF`;
});
// 14880768 (DUMMY)
...
CP0Auto.insertHex('f4a0', 13, (slice, indent) => {
    ...
    if (push) { // f4a4
        ...
        let decompiled;
        try {
            decompiled = (0, disassembler_1.decompileMethodsMap)(subslice.clone(), indent);
        }
        catch (e) {
...
//# sourceMappingURL=cp0.generated.js.map